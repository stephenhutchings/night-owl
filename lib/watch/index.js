  ...
  if (type === "change") {
    ...
    if (dependants.length) {
      ...
      if (dependencyCache[filename] !== content) {
        ...
        changed.render = dependants
          .map((t) => t.pages.map((p) => p.page.url))
          .flat()
      }
    }
  }

  if (!changed.render.length && (type === "change" || type === "add")) {
    ...
    const watcher = chokidar
      ...
      .on("all", (type, filename) =>
        handle(type, filename, templates, config).then((changed) => {
          if (changed.render.length) {
            dorender().then(() => onChange(changed.render))
          } else if (changed.reload.length) {
